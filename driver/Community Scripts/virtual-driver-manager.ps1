...
try {
    ...
    if ($Action -in @('install', 'uninstall')) {
        ...
        if ($Action -eq 'install') {
            Write-Log -Message "Starting driver installation from local files..."
            $infPath = Join-Path (Split-Path -Parent $PSScriptRoot) "VirtualDisplayDriver\MttVDD.inf"
            
            if (-not (Test-Path $infPath)) {
                throw "Local driver INF file not found at $infPath"
            }

            Write-Verbose "Running DevCon to install the driver locally..."
            & $devconExe install $infPath "Root\MttVDD"
        }
